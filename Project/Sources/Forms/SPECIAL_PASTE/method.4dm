  ...
  // ----------------------------------------------------
C_LONGINT:C283($i;$kLon_columns;$kLon_tools;$Lon_event;$Lon_options)
				...
				: (<>tTxt_Labels=7)  // JSON
					...
					Case of 
						...
						: (Form:C1466.original="[@]")
							  ...
							  //
							Form:C1466.text:=codeForCollection (JSON Parse:C1218(Form:C1466.original))
						...
						: (Form:C1466.original="{@}")
							...
							Form:C1466.text:=codeForObject (JSON Parse:C1218(Form:C1466.original))
					...
					End case 
					  ...
					  //………………………………………………………………………………………………………
					  //: (<>tTxt_Labels=7)  // UNICODE (adapted from Miyako) [OBSOLETE SINCE CODE EDITOR IS UTF-8]
					  //$t:=Form.original
					  //Form.text:=""
					  //$i:=1
					  //ARRAY LONGINT($tLon_position;0x0000)
					  //ARRAY LONGINT($tLon_lengths;0x0000)
					  //ARRAY LONGINT($tLon_specialCodePoints;0x0000)
					  //While (Match regex("([!-~]+)|([^!-~]+)";$t;$i;$tLon_position;$tLon_lengths))
					  //If ($tLon_lengths{1}#0)
					  //$Txt_plain:=Substring($t;$tLon_position{1};$tLon_lengths{1})
					  //$Txt_encoded:=$Txt_encoded+Choose($Txt_encoded="";"";"+")+Choose($Txt_plain="";"";"\"")+$Txt_plain+Choose($Txt_plain="";"";"\"")
					  //$i:=$tLon_position{1}+$tLon_lengths{1}
					  //Else
					  //$Txt_source:=Substring($t;$tLon_position{2};$tLon_lengths{2})
					  //For ($Lon_ii;1;Length($Txt_source);1)
					  //$Lon_specialCodePoint:=Character code(Substring($Txt_source;$Lon_ii;1))
					  //If (Find in array($tLon_specialCodePoints;$Lon_specialCodePoint)=-1)
					  //APPEND TO ARRAY($tLon_specialCodePoints;$Lon_specialCodePoint)
					  //End if
					  //$Lon_x:=($Lon_x+1)*Num($Lon_x<5)
					  //$Txt_encoded:=$Txt_encoded+Choose($Lon_x=5;"\\\r";"")+Choose($Txt_encoded="";"";"+")+"$ICU_"+Substring(String($Lon_specialCodePoint;"&x");3;4)
					  //End for
					  //$i:=$tLon_position{2}+$tLon_lengths{2}
					  //End if
					  //End while
					  //For ($i;1;Size of array($tLon_specialCodePoints);1)
					  //Form.text:=Form.text\
						+"$ICU_"+Substring(String($tLon_specialCodePoints{$i};"&x");3;4)\
						+":="+Command name(90)+"(0x"+Substring(String($tLon_specialCodePoints{$i};"&x");3;4)+")"\
						+"\r"
					  //End for
					  //Form.text:=Form.text+"\r$Txt_unicode:="+$Txt_encoded+"\r"
