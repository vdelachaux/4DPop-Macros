  ...
  // ----------------------------------------------------
  // Declarations
C_LONGINT:C283($kLon_COUNT;$Lon_formEvent;$Lon_i;$Lon_options;$Lon_unused)
...
Case of 
	...
	: ($Lon_formEvent=On Load:K2:1)
		...
		If (True:C214)  // beautifier
			...
			For ($Lon_i;1;$kLon_COUNT;1)
				...
				$Ptr_options->{$Lon_i}:=$Lon_options ?? ($Lon_i-1)
				$Ptr_labels->{$Lon_i}:=Get localized string:C991("beautifier_"+String:C10($Lon_i-1;"00"))
				
			End for 
			
			ARRAY LONGINT:C221($tLon_sortOrder;$kLon_COUNT)
			...
			SORT ARRAY:C229($tLon_sortOrder;$Ptr_options->;$Ptr_labels->)
	...
	: ($Lon_formEvent=On Unload:K2:2)
		...
		If (True:C214)  // beautifier
			...
			For ($Lon_i;1;Size of array:C274($Ptr_options->);1)
				
				If ($Ptr_options->{$Lon_i})
					
					$Lon_options:=$Lon_options ?+ ($Lon_i-1)
